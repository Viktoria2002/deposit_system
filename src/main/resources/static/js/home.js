window.onload = function () {
    ...
    $(document).ready(function() {
        $.getJSON('/clientHome').done(function (data) {
            if(data.length != 0) {
                $.each(data, function (key, item) {
                    text.innerHTML = '<p>По вашему срочному вкладу "' + item.termDeposit['depositName']+ '" истек срок хранения. Пожалуйста закройте вклад в разделе "Мои вклады".</p>';
                });
